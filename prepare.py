...
def clean_telco(df):
    ...
    # Drop gender (exporation shows it's nearly evenly split and therefore not likely a driver of churn)
    ...
    df['streamer'] = (((df['streaming_tv_Yes'] == 1) & (df['streaming_movies_Yes'] == 1)) | ((df['streaming_tv_Yes'] == 1) | (df['streaming_movies_Yes'] == 1))) 
   ...
   # Create new columns for payment types 
    df['use_Check'] = (df['payment_type_id'] == 1)
